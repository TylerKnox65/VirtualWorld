...
import sqlite3
class dataBase():
        def __init__(self) -> None:#creates database
                file = 'dbase.db'
                self.connection = sqlite3.connect(file)
                self.cursor = self.connection.cursor()
                query = """
                create table if not exists adventureworld (
                ...
                if len(self.fetch()) == 0:
                        query = f"insert into adventureworld (position) values ('[0,0]');"
                        self.cursor.execute(query)
                        self.connection.commit()
                
                
        def updateBase(self,coords):
                print(f"coords {coords}")
                query = f"UPDATE adventureworld SET position = '{coords}' WHERE id = 1;"
                self.cursor.execute(query)
                self.connection.commit()
                #print("UPDATE")
        def fetch(self):
                query = f"select position from adventureworld"
                self.cursor.execute(query)
...
class world():
        ...
        def moveit(self):
                ...
                while True:
                                ...
                                #self.coordinates = [i for i in dataBase().fetch()]
                                for i in dataBase().fetch():
                                        for o in i:

                                                self.coordinates = o.strip("][").split(',')
                                                self.coordinates = [int(i) for i in self.coordinates]
                                ...
                                dataBase().updateBase(self.coordinates)
        ...
        def cave(self):
                while True:
                        self.pickaxecheck = [i for i in self.inventory if i == "pickaxe"]
                        
                        self.cavechoice = str(input("Do you want to enter the cave?: ").lower())
                        if self.cavechoice == "yes":
                                if "pickaxe" in self.inventory:
                                        self.miningchoice = str(input("You can mine iron, coal, and stone\nWhat would you like to mine?: ").lower())
                                        if self.miningchoice in self.minerals:
                                                self.mineralamount = int(input("How much do you want?: "))
                                                for i in self.minerals:
                                                        if self.miningchoice == i:
                                                                self.inventory[i] = self.mineralamount
                                                                

                                else:
                                        print("if you don't have a pickaxe then why are you here")      
                                        break          
                        else:
                                print("then piss off")
                                break
